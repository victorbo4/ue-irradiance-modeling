...
void USunSkyController::ApplyStaticConfig(double Latitude, double Longitude, double Timezone, double NorthOffset, bool bUseDST)
{
    if (AActor* SunSky = EnsureSunSky())
    {
        ...
        SetDoubleProp(SunSky, TEXT("TimeZone"), Timezone);
        ...
        float TZone = 0.f;
        bSuccess &= GetFloatProp(SunSky, TEXT("TimeZone"), TZone);
        PYRANO_INFO(TEXT("[SunSky] Timezone = %.3f"), TZone);
...
{
    // Convert incoming UTC to Local time (UTC + TimeZone)
    // because the SunSky actor expects 'SolarTime' in local clock format. 

    // The actor will internally subtract the configured TimeZone 
    // (set in ApplyStaticConfig) to resolve the correct physical sun position.
