...
void UPyranoEditorSubsystem::ScheduleStartAndMonitor(UWorld* PlayWorld)
{
    ...
    constexpr float BootDelaySec = 4.25f;
        ...
        [this]()
        {
            ...
            if (auto* Scheduler = PW->GetSubsystem<UIrradianceScheduler>())
            {
                ...
                else
                {
                    ...
                    Scheduler->CaptureOnce(PendingSimConfig);
                }
            }
...
{
    // LocalTime -> UTC
    FSimConfig SimConfigUTC = InConfig;
    FTimespan TimeZoneOffset = FTimespan::FromHours(InConfig.Timezone);
    SimConfigUTC.StartTime = InConfig.StartTime - TimeZoneOffset;
    SimConfigUTC.EndTime = InConfig.EndTime - TimeZoneOffset;
    SimConfigUTC.Timezone = InConfig.Timezone;

    StartPIEWithConfig(SimConfigUTC, /*bIsSimulation=*/false);
}
...
void UPyranoEditorSubsystem::StartSimulation(const FSimConfig& InConfig)
{
    ...
    StartPIEWithConfig(SimConfigUTC, /*bIsSimulation=*/true);
}
...
bool UPyranoEditorSubsystem::BrowseForOutputFolder(FDirectoryPath& InOutPath)
{
    ...
    FString DefaultPath = FPaths::ProjectSavedDir() / TEXT("Irradiance");
    const bool bSelected = Desktop->OpenDirectoryDialog(ParentWindowHandle, TEXT("Select Output Folder"), DefaultPath, OutFolder);
