...
void UConsoleLogWidget::ToggleShowVerbose()
{
    ...
    if (bShowVerbose)
    {
        LogPyrano.SetVerbosity(ELogVerbosity::VeryVerbose);
        IrradianceCommon::Console::SetConsoleVerbosityLimit(ELogVerbosity::VeryVerbose);
        PYRANO_INFO(TEXT("[Console] Verbose logs ENABLED (Category=%d, Limit=%d)"),
            (int32)LogPyrano.GetVerbosity(),
            (int32)IrradianceCommon::Console::GetConsoleVerbosityLimit());
    }
    else
    {
        LogPyrano.SetVerbosity(ELogVerbosity::Log);
        IrradianceCommon::Console::SetConsoleVerbosityLimit(ELogVerbosity::Log);
        PYRANO_INFO(TEXT("[Console] Verbose logs DISABLED (Category=%d, Limit=%d)"),
            (int32)LogPyrano.GetVerbosity(),
            (int32)IrradianceCommon::Console::GetConsoleVerbosityLimit());
    }
}
...
void UConsoleLogWidget::ExportLogToFile()
{
    ...
    if (Items.Num() == 0)
    {
        PYRANO_CONSOLE(TEXT("[Console] Nothing to export"));
        return;
    }
    ...
    if (!DesktopPlatform)
    {
        PYRANO_ERR(TEXT("[Console] DesktopPlatform not available."));
        return;
    }
